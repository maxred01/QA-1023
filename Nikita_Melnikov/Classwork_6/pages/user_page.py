...
class UsersPage:
    ...
    def __init__(self):
        ...
        self.sessions.headers.update({'Accept': 'application/json'})  # если апи имеет специфический заголовок
        self.users_url = f"{BASE_URL}/users"

    def _requests(self, method, url, **kwargs):
        try:
            response = self.sessions.request(method, url, **kwargs)
            response.raise_for_status()  # Поднимает HTTP ошибку если статуст ответа 400 или 500
            return response
    ...
    def create_user(self, data):
        ...
        return self._requests('POST', self.users_url, timeout=5)

    def update_user(self, user_id, data):
        """Обновление данных пользователя по ID"""
        return self._requests('PUT', self.users_url, timeout=5)

    def delete_user(self, user_id):
        """Удаление пользователя по ID"""
        return self._requests('DELLETE', self.users_url, timeout=5)
